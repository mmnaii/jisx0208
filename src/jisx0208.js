import {rowcell} from "./table/rowcell.js";
import {jisx0201kana} from "./table/jisx0201kana.js";

function toUint8(a) {
	return Number(a) >>> 0 & 255;
}

function isAssigned(row, cell) {
	/*
		Returns true if the JIS X 0208 assigns a charactor to the {row, cell}.
	*/
	return Boolean(getCharacter(row, cell));
}

function getCharacter(row, cell) {
...
function sjisToRowCell(c1, c2, noAssert) {
	/*
	...
	*/
	...
	if ((0x81 <= c1 && c1 <= 0x9f || 0xe0 <= c1 && c1 <= 0xef) && (0x40 <= c2 && c2 <= 0xfc && c2 != 0x7f)) {
		...
		return noAssert || isAssigned(row, cell) ? [row, cell] : null;
	} else {
		return null;
	}
}

function rowCellToSjis(row, cell, noAssert) {
	/*
	...
	*/
	...
	if (!noAssert && !isAssigned(row, cell)) {
		return null;
...
function eucjpToRowCell(c1, c2, noAssert) {
	/*
...
function rowCellToEucjp(row, cell, noAssert) {
	/*
		Number	row,			The row of the JIS X 0208.
		Number	cell,			The cell of JIS X 0208.
		Boolean	noAssert,		Skip the validation of assignment at the row-cell?
...
function halfkanaToRowCell(code) {
	/* 
...
export {Jisx0208 as default, isAssigned, getCharacter, sjisToRowCell, rowCellToSjis, eucjpToRowCell, rowCellToEucjp, halfkanaToRowCell};
